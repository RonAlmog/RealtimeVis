...
namespace RealtimeAngular
{
    public class BackgroundTicker : IRegisteredObject
    {
        ...
        public double bandwidththPct = 20.0;
        ...
        {
            hub.Clients.All.broadcastMessage(DateTime.UtcNow.ToString(), bandwidththPct, cpuPct, salesAmt, alphaSalesAmt, betaSalesAmt);

            // update values
            Random r = new Random();
            bandwidththPct += 15 * r.NextDouble() - 7.5;
            if (bandwidththPct > 100) bandwidththPct = 100;
            if (bandwidththPct < 0) bandwidththPct = 0;
